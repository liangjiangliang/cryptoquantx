...
const DataLoadModal: React.FC<DataLoadModalProps> = ({isOpen, onClose, onLoadData}) => {
    ...
    // 监听自定义事件以设置默认值
    useEffect(() => {
        const handleSetDefaultValues = (event: CustomEvent) => {
            if (event.detail) {
        ...
        window.addEventListener('setDefaultDataLoadValues', handleSetDefaultValues as EventListener);

        return () => {
            window.removeEventListener('setDefaultDataLoadValues', handleSetDefaultValues as EventListener);
        };
    }, []);
